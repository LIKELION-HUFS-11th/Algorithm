studentNum = int(input(""))

result = []

for i in range(studentNum):
    studentNameAndScore = input("")
    studentNameAndScore = studentNameAndScore.split(" ")
    if not result:
        result.append(studentNameAndScore)
    else:
        j = 0
        while j < len(result):
            if int(studentNameAndScore[1]) > int(result[j][1]):   
                result.insert(j, studentNameAndScore)
                break
            elif int(studentNameAndScore[1]) == int(result[j][1]):
                k = j
                while k < len(result) and int(studentNameAndScore[1]) == int(result[k][1]):
                    if int(studentNameAndScore[2]) < int(result[k][2]):
                        result.insert(k, studentNameAndScore)
                        break
                    elif int(studentNameAndScore[2]) == int(result[k][2]):
                        l = k
                        while l < len(result) and int(studentNameAndScore[2]) == int(result[l][2]):
                            if int(studentNameAndScore[3]) > int(result[l][3]):
                                result.insert(l, studentNameAndScore)
                                break
                            elif int(studentNameAndScore[3]) == int(result[l][3]):
                                a = l
                                while a < len(result) and int(studentNameAndScore[3]) == int(result[a][3]):
                                    if studentNameAndScore[0] < result[a][0]:
                                        result.insert(a, studentNameAndScore)
                                        break
                                    elif a == len(result)-1:
                                        result.append(studentNameAndScore)                                
                                    a += 1
                            l+=1
                    k+=1
            j+=1

for b in range(len(result)):
    print(result[b][0])

